...
class UpstashRedisClient:
    ...
    def scan(self, cursor: int = 0, match: str = None, count: int = 10, _type: str = None) -> tuple:
        ...
        Returns (new_cursor, keys_list)
        
        This is a simplified SCAN implementation for Upstash.
        Since Upstash doesn't support SCAN directly via REST, we use KEYS as fallback.
        """
        try:
            # Use KEYS command as fallback (note: SCAN is preferred for large datasets)
            if match:
                # Convert Redis pattern to match
                keys = self.keys(match)
            else:
                keys = self.keys("*")
            
            # Return (cursor=0 to indicate completion, keys_list)
            # This is a simplified implementation - in production with large datasets,
            # you'd want to implement true pagination
            return 0, keys
        except Exception as e:
            logger.warning(f"Error scanning keys: {e}")
            return 0, []
    
    def pipeline(self):
        """
        Create a pipeline for batching multiple commands.
        Returns a PipelineClient that batches commands.
        """
        return UpstashPipelineClient(self)


class UpstashPipelineClient:
    """
    Pipeline client for batching multiple Redis commands to Upstash.
    """
    ...
    def execute(self) -> list:
        ...
        for cmd_name, cmd_args in self.commands:
            try:
                if cmd_name == 'DELETE':
                    result = self.client.delete(cmd_args)
                elif cmd_name == 'SET':
                    key, value, ex = cmd_args
                    result = self.client.set(key, value, ex)
                elif cmd_name == 'GET':
                    result = self.client.get(cmd_args)
                else:
                    result = None
                results.append(result)
            except Exception as e:
                logger.warning(f"Pipeline command failed: {cmd_name} - {e}")
                results.append(None)
