...
@product_details_bp.route('/explore', methods=['GET'])
def get_explore_products():
    """
    Get random products for 'Explore Your Interest' section.
    Returns up to 20 random products with lightweight serialization.
    ...
    cache_key = f"product:explore:{limit}"
    request_timestamp = datetime.utcnow().isoformat()
    cache_hit = False
    
    try:
        # Try cache first
        cached_explore = product_cache.get(cache_key)
        
        if cached_explore:
            try:
                explore_data = json.loads(cached_explore) if isinstance(cached_explore, str) else cached_explore
                cache_hit = True
                logger.debug(f"CACHE HIT: {cache_key}")
            except Exception as e:
        ...
        # Cache miss - fetch from database
        if not cache_hit:
            # Get random active products with eager loading
            explore_products = Product.query.options(
            ...
            # Serialize products with lightweight format
            explore_data = []
            ...
            # Cache the explore products data
            try:
                cache_ttl = CACHE_TTL.get('explore_products', 600)
        ...
        logger.error(f"Error in get_explore_products: {e}", exc_info=True)
        return jsonify({
            'success': False,
            'error': 'Internal server error',
            'timestamp': request_timestamp
