...
export default function CacheGroupsDisplay({ status, isLoading }: CacheGroupsDisplayProps) {
  return (
    <div>
      <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-slate-300">Cache Groups</h2>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        {CACHE_GROUPS.map((group) => (
          <div
            key={group.pattern}
            className={`group rounded-lg border-2 bg-gradient-to-br ${group.color} ${group.borderColor} p-4 transition-all hover:border-opacity-100`}
          >
            <div className="flex items-start justify-between mb-3">
              <Database className="h-5 w-5 text-slate-400 group-hover:text-slate-300" />
              <Eye className="h-4 w-4 text-slate-500 opacity-0 transition-opacity group-hover:opacity-100" />
            </div>
            ...
            <div className="mt-3 flex items-center justify-between">
              <code className="text-xs text-slate-500 font-mono">{group.pattern}</code>
              <Button
                variant="ghost"
                size="sm"
                className="h-6 w-6 p-0 text-slate-400 hover:bg-white/5 hover:text-red-400"
              >
