...
const MAX_TOP_PICKS_PRODUCTS = 10
...
export function TopPicksClient({ initialProducts }: TopPicksClientProps) {
  const products = useMemo(() => (initialProducts ?? []).slice(0, MAX_TOP_PICKS_PRODUCTS), [initialProducts])
      ...
      <div className="w-full">
        <div className="flex items-center justify-between bg-[#8B1538] px-2 py-1.5 text-white sm:px-4 sm:py-2">
          <div className="flex items-center gap-1 sm:gap-2">
            <Star className={`h-4 w-4 fill-yellow-300 text-yellow-300 sm:h-5 sm:w-5`} />
